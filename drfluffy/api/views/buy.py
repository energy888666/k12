...
class BuyView(APIView):
    ...
    def _get_coupon_list(self,request,course_id=None):
        '''该方法用于返回用户课程购物券或者通用的购物券信息(course_id=None)'''
        ...
        coupon_list = models.CouponRecord.objects.filter(
            ...
            coupon__object_id=course_id,  # 限定哪一个课程(为None时为通用优惠券)
        ...
        for item in coupon_list:
            coupon_dict = {
                ...
                "valid_begin_date": datetime.datetime.strftime(item.coupon.valid_begin_date, "%Y-%m-%d %H:%M:%S"),
            # 有效开始时间
